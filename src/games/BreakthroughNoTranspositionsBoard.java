...
public class BreakthroughNoTranspositionsBoard extends OnePieceTypeTwoPlayerBoard implements UndoBoard, AlphaZeroBoard, PieceCountBoard, RectangularIntBoard {
    ...
    protected long[][][][] zobristKeys;

    public BreakthroughNoTranspositionsBoard(Breakthrough game) {
        super(game);
    ...
    public BreakthroughNoTranspositionsBoard(Breakthrough game, String board, boolean isFileName) {
        super(game);
        zobristKeys = game.getZobristKeys();
        if(isFileName) {
            try {
    ...
    public void copyDataFrom(Board thatboard) {
        ...
        BreakthroughNoTranspositionsBoard that = (BreakthroughNoTranspositionsBoard) thatboard;
        for(int x=0;x<board.length;x++) {
    ...
    public boolean equals(Object o) {
        ...
        BreakthroughNoTranspositionsBoard that = (BreakthroughNoTranspositionsBoard) o;
    ...
    public long[] getMovesHashForColor() {
        return movesHashForColor;
    }

    public long[] getPiecesHashForColor() {
        return piecesHashForColor;
    }
    ...
    protected void initialiseStartingBoard(int width, int height, int rowsOfPieces, int startingColor) {
    ...
    protected void initializeBoardFromString(String boardString, int boardWidth, int boardHeight, int rowsOfPieces) {
    ...
    protected void initializeLegalMoves() {
        if(Util.isDebug()) System.out.println("Computing legal moves");
        ...
        for(int y=0;y<board[0].length;y++) {
            for(int x=0;x<board.length;x++) {
                ...
                if(piece!=NO_PIECE) {
                    ...
                    //check the three possible locations a piece could move to.
                    //legal moves aren't needed anymore when a player has reached his opponent's home row, so there are always potential moves.
                    //moving diagonally is allowed iff the to-field is on the board and not occupied by yourself.
                    //moving straight is allowed iff the to-field is empty.
                    if(getSquare(x,y+moveDirection)==NO_PIECE) { //move straight ahead possible?
                        legalMoves.add(new BreakthroughMove(x,y,x,y+moveDirection,false));
                    }
                    if(x<board.length-1) {
                        if(getSquare(x+1,y+moveDirection)==NO_PIECE) {
                            legalMoves.add(new BreakthroughMove(x,y,x+1,y+moveDirection,false));
                        } else if(getSquare(x+1,y+moveDirection)!=OFF_BOARD && getSquare(x+1,y+moveDirection)!= getSquare(x,y)) {
                            legalMoves.add(new BreakthroughMove(x,y,x+1,y+moveDirection,true));
                        }
                    }
                    if(x>0) {
                        if(getSquare(x-1,y+moveDirection)==NO_PIECE) {
...
//        movesHashForColor[m.getColorOfMove()] ^= moveHash(m, getTurn());
    ...
    @Override
    public Map<Integer, String> shortCSVOfAllBoardSymmetries() {
        Map<Integer, String> result = new HashMap<>();
        String plainBoard = "";
        for(int y=board[0].length-1;y>=0;y--) {
            for(int x=0;x<board.length;x++) {
                plainBoard += board[x][y]+",";
            }
        }
        plainBoard += getColorToPlay()==WHITE ? "1," : "0,";
        plainBoard += getTurn()+",";
        ...
        for(int y=board[0].length-1;y>=0;y--) {
            for(int x=board.length-1;x>=0;x--) {
                verticalSymmetry += board[x][y]+",";
            }
        }
        verticalSymmetry += getColorToPlay()==WHITE ? "1," : "0,";
        verticalSymmetry += getTurn()+",";
        result.put(2, verticalSymmetry);
        return result;
    }
    ...
    public Move toMove(String moveString) throws MalformedMoveException {
        return new BreakthroughMove(moveString);
    }
